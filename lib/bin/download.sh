...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/codecrafters-io/cli.git}"
    ...
    url="https://github.com/codecrafters-io/cli/releases/download/v{version}/v{version}_{os}_{arch}.tar.gz"
    url="$(kc_asdf_template "$url" "${vars[@]}")"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/codecrafters-io/cli/releases/download/v{version}/cli_{version}_checksums.txt"
